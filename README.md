...
Visually, this also offers a practical spectral view of the reverse process for arbitrary densities learned by diffusion models. As an example, for a simple variance-preserving (Song '21) reverse diffusion one can visualize the eigenfunctions of the Score Hamiltonian and the associated eigenspectrum to analyze the dynamics of the spectrum and its gap $\Delta(t)$ through diffusion time. The ground state represents the density, the first excited state ${\psi}_{1}$ the hardest bottleneck mode of the density, and the higher excited states ${\psi}_{k \geq 2}$ representing the higher-order modes resolved during diffusion:
